...
> В сязи с этим вы можете наблюдать ошибку `Error response from daemon: client version 1.25 is too old. Minimum supported API version is 1.44, please upgrade your client to a newer version` при запуске контейнера.
> Eсли хоите и дальше продолжать использовать Wathtower, то можете воспользоваться форками от комьюнити.
> Либо воспользоваться более интересной и функциональной альтернативой - [Komodo](/blog/komodo-upravlenie-conteynerami-i-stekami-docker).
